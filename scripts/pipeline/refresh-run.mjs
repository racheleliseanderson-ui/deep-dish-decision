...
function cooldownDays(item, attempt) {
  ...
  if (
    SUCCESS.has(status) &&
    item.reasons.some((r) => r.startsWith("thin-") || r.startsWith("review-"))
  ) {
    return 7;
  }
  return 1;
...
function eligible(item) {
  ...
  if (FORCE) return { yes: true, eligibleAt: null, cooldown: 0, attempt };
  const cooldown = cooldownDays(item, attempt);
  if (!attempt.last?.at || cooldown <= 0) {
    return { yes: true, eligibleAt: null, cooldown, attempt };
  }
...
const scored = queue.items
  .filter((item) => item.priority > 0)
  .map((item) => ({ item, gate: eligible(item) }));
const candidates = scored.filter(({ gate }) => gate.yes);
const held = scored.filter(({ gate }) => !gate.yes);
...
console.log(
  `Refresh queue: ${queue.totals.dueNow} due; ${candidates.length} eligible now; ${held.length} cooling down.`,
);
console.log(
  `Selected ${selected.length} (batch ${BATCH}, max ${PER_CITY} per city before fill):`,
);
for (const [i, row] of selected.entries()) {
  console.log(
    `  ${String(i + 1).padStart(2)}. ${row.item.slug}  ${row.item.reasons.join(", ")}`,
  );
}
if (held.length) {
  ...
  for (const row of held.slice(0, 10)) {
    console.log(
      `  - ${row.item.slug} until ${row.gate.eligibleAt} (${row.gate.attempt.streak} failure attempts)`,
    );
  }
}
...
writeJson(path.join(process.cwd(), "reports/refresh-plan-latest.json"), {
  ...
  coolingDown: held.map(({ item, gate }) => ({
    ...item,
    eligibleAt: gate.eligibleAt,
    cooldownDays: gate.cooldown,
  })),
});
...
const result = spawnSync(
  process.execPath,
  ["scripts/pipeline/enrich.mjs", `--slugs=${slugs}`],
  { stdio: "inherit" },
);
...
const refresh = spawnSync(
  process.execPath,
  ["scripts/pipeline/refresh.mjs", "--print=15"],
  { stdio: "inherit" },
);
