...
export function markFamily(r: RestaurantRecord): MarkFamily {
  ...
  if (
    tags.some((t) => /omakase|tasting|kaiseki|degustation/.test(t)) ||
    /tasting|immersive/.test(service)
  )
    return "tasting";
  if (tags.some((t) => /sushi|sashimi|raw|oyster|crudo|ceviche/.test(t))) return "raw";
  if (
    tags.some((t) => /steak|grill|bbq|wood-fired|asador|churrasco/.test(t)) ||
    /grill|hearth/.test(atmo)
  )
    return "grill";
  if (
    tags.some((t) => /bar|cocktail|wine|tapas|small plate/.test(t)) ||
    /bar-led|late/.test(service)
  )
    return "bar";
