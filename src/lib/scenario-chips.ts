...
export function scenarioChips(s: Situation): ScenarioChip[] {
  ...
  if (s.partySize != null) out.push({ id: "party", label: `Party of ${s.partySize}`, tone: "neutral" });
  if (s.leadDays != null) {
    out.push({
      id: "lead",
...
export function conditionChips(r: RestaurantRecord, s: Situation, blocked: boolean): ScenarioChip[] {
  const out: ScenarioChip[] = [];
  ...
  if (s.constraints.some((c) => /mobility|step-free|access/i.test(c)) && /not stated|unstated|unknown/i.test(r.accessibilityState ?? ""))
    out.push({ id: "access-hold", label: "Access evidence thin", tone: "watch" });
  if (s.constraints.some((c) => /private/i.test(c)) && !/private|semi-private/i.test(`${r.groupDetails ?? ""} ${r.atmosphereSummary ?? ""}`))
