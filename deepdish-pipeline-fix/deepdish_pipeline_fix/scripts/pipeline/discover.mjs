...
if (!apiKey) {
  ...
  console.error("Add GOOGLE_MAPS_API_KEY=... to .env.local in the repository root, then run again.");
  process.exit(2);
}
...
function cityKey(city, stateCode) {
  return `${String(city).trim().toLowerCase()}|${String(stateCode).trim().toUpperCase()}`;
}
...
function targetsFor() {
  ...
  return pending
    .slice()
    .sort((a, b) => Number(Boolean(b.pinned)) - Number(Boolean(a.pinned)) || a.priority - b.priority || b.gap - a.gap)
    .slice(0, CITY_COUNT);
}
    ...
    try {
      res = await fetch("https://places.googleapis.com/v1/places:searchText", {
        ...
        headers: {
          ...
          "X-Goog-FieldMask": "places.id,places.displayName,places.websiteUri,places.primaryType,nextPageToken",
        },
        body: JSON.stringify({
          textQuery: query,
...
for (const market of targets) {
  ...
  for (const seed of seeds) {
    ...
    searches += 1;
    if (!result?.ok) {
      misses.push({ market: `${market.city}, ${market.stateCode}`, query, reason: result?.error || "search failed" });
      continue;
    }
  ...
  console.log(`${market.city}, ${market.stateCode}: ${seen.size} surfaced, ${listings.length} first-party verified for a gap of ${market.gap}.`);
  if (listings.length) {
